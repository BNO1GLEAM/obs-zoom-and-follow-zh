...
description = (
    "如何使用（如果显示不完全，请设置脚本窗口大小）：\n\n"
    "1.选择缩放源（如果没有，请检查OBS来源中是否设置采集源）。\n\n"
    "2.启用监视器覆盖。\n\n"
    "3.设置缩放的长宽高等数值。\n\n"
    "4.在OBS来源中选择第一步中设置的画面源，按下Ctrl+E。\n\n"
    "5.设置对起点为居中，将画面重新拉满覆盖，之后选择边框类型等比缩放到框内，框内对齐方式为居中。\n\n"
    "6.最后到OBS设置——热键中搜索zoom，设置一个热键\n\n"
    + "By tryptech (@yo_tryptech / tryptech#1112) 汉化B站：头文ZD\n\n"
    "github.com/tryptech/obs-zoom-and-follow \n\n"
    + f"{version}"
)
...
ZOOM_DESC_TOG = "zoom插件 启用/禁用鼠标缩放"
FOLLOW_DESC_TOG = "zoom插件 启用/禁用鼠标跟随"
...
def script_properties():
    ...
    mon_size = obs.obs_properties_add_bool(props,
                                           "Manual Monitor Dim", "启用监控覆盖尺寸")

    mon_w = obs.obs_properties_add_int(props,
                                       "Monitor Width", "手动设置显示器宽度", -8000, 8000, 1)
    mon_h = obs.obs_properties_add_int(props,
                                       "Monitor Height", "手动设置显示器高度", -8000, 8000, 1)

    offset = obs.obs_properties_add_bool(props,
                                         "Manual Offset", "启用手动偏移")

    mx = obs.obs_properties_add_int(props,
                                    "Manual X Offset", "手动X偏移", -8000, 8000, 1)
    my = obs.obs_properties_add_int(props,
                                    "Manual Y Offset", "手动Y轴", -8000, 8000, 1)

    obs.obs_properties_add_int(props,
                               "Width", "缩放窗口宽度", 320, 3840, 1)
    obs.obs_properties_add_int(props,
                               "Height", "缩放窗口高度", 240, 3840, 1)
    obs.obs_properties_add_float_slider(props,
                                        "Border", "活动边框（默认0.15）", 0, 0.5, 0.01)
    obs.obs_properties_add_int(props,
                               "Speed", "鼠标移动时跟随速度（默认160 值越小速度越慢）", 0, 540, 10)
    obs.obs_properties_add_float_slider(props,
                                        "Smooth", "鼠标移动时动画平滑（默认1.0 值越大动画效果越弱）", 0, 10, 0.1)
    obs.obs_properties_add_int_slider(props,
                                      "Zoom", "缩放时动画持续时间 (ms) 值越大动画越慢", 0, 1000, 1)
